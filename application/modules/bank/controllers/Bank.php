...
class Bank extends MX_Controller
{
    ...
    public function bdtask_3rdPartybanks()
    {
        $data['module']   = "bank";
        $data['page']     = "manage3rd_party_bank";
        echo Modules::run('template/layout', $data);
    }

    public function bdtask_3rdPartybranches()
    {
        $data['module']   = "bank";
        $data['page']     = "manage3rd_party_branch";
        echo Modules::run('template/layout', $data);
    }
    ...
    {
        $this->db->select('3rdpartybranch.id, 3rdpartybank.bankname, 3rdpartybranch.branchname');
        $this->db->from('3rdpartybranch');
        $this->db->join('3rdpartybank', '3rdpartybank.id = 3rdpartybranch.bankid', 'left');
        $this->db->order_by('3rdpartybank.bankname', 'ASC');
        $query = $this->db->get();
        $result = $query->result();
        echo json_encode($result);
    }

    public function getBranchesById()
    {
        $this->db->select('3rdpartybranch.id, 3rdpartybank.bankname, 3rdpartybranch.branchname');
        $this->db->from('3rdpartybranch');
        $this->db->join('3rdpartybank', '3rdpartybank.id = 3rdpartybranch.bankid', 'left');
        $this->db->where('3rdpartybank.id', $this->input->post('bank', true));
        $query = $this->db->get();
        $result = $query->result();
        echo json_encode($result);
    }
    ...
    {
        $bankData = array(
            'bankname'      => $this->input->post('bank', true)
        );
        $this->db->insert('3rdpartybank', $bankData);
        echo json_encode("save sucessfully");
    }
